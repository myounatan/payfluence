...
app.get('/allowance/:airdropId/:fid', async (c) => {
  try {
    ...
    let cachedAllowance = Number(await c.env.ALLOWANCE_KV.get(cacheKey));
    ...
    const allowance = Math.ceil((boost + reactionScore) * 500);

    console.log(boost, reactionScore, allowance)
    ...
    return new Response(
      JSON.stringify({
        ...
        message: "Daily allowance found",
        data: {
          allowance: Math.ceil((boost + reactionScore) * 500)
        }
      }),
      { status: 200 }
    );
